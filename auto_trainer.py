...
def run_auto_trainer():
    ...
    # וידוא טיפוסים
    log_message(f"type(SECTORS_LIST): {type(SECTORS_LIST)}")
    for i, (slot, tickers) in enumerate(SECTORS_LIST):
        log_message(f"Sector {i}: slot type={type(slot)}, slot value={slot}, tickers type={type(tickers)}")
    ...
    try:
        for sector_idx in range(total_sectors):
            ...
            write_status(
                state="running", message=f"מעבד סקטור: {slot}",
            ...
            # === נקודת התיקון: וידוא ש-slot הוא מחרוזת לפני הניקוי ===
            slot_str = str(slot)
            safe_slot_name = clean_filename(slot_str)
            log_message(f"clean_filename returned: {safe_slot_name} (type: {type(safe_slot_name)})")
            ...
            meta = {
                ...
                "train_acc": train_acc, "test_acc": train_acc,
                "slot": slot, "model_type": "Wyckoff-Anchored",
                "num_trades": len(combined_df), "recommended_threshold": optimal_th
            }
