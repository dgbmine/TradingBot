...
def read_batch_config():
    """
    מחזיר dict עם {slot, tickers} אם קיים batch_config.json תקין,
    אחרת None (→ ריצה מלאה).
    """
...
def train_sector(slot, tickers, start_date, end_date, base_threshold=35, risk_profile="Aggressive"):
    """
    מבצע Backtest על כל ticker ברשימה, אוסף features + labels.
    מחזיר: (features_list, added_trades, errors)
    """
    features_list = []
    errors = 0
    added_trades = 0
    ...
    for idx, ticker in enumerate(tickers):
        ...
        try:
            ...
            # ── הוספת מאקרו (SPY/VIX) לנתוני ה-BT ──
...
def build_and_save_model(slot, combined_df, started_at):
    """
    מקבל DataFrame מאוחד, מאמן RandomForest, שומר pickle + CSV.
    מחזיר True בהצלחה, False אם נכשל.
    """
    ...
    # ── שמירת PKL (אמינה עם tmp + replace) ──
...
def run_auto_trainer():
    ...
    # ── רף איסוף נמוך כדי לאסוף מספיק דוגמאות ──
    base_threshold = 35

    # ── קריאת batch_config (אם קיים) ──
