...
if st.button("Generate Investment Brief"):
    ...
    # הגנות קריטיות לחישוב פיבוטים
    resistance = float(np.max(piv_highs[-5:])) if len(piv_highs) >= 5 else float(df["High"].max())
    support = float(np.min(piv_lows[-5:])) if len(piv_lows) >= 5 else float(df["Low"].min())
