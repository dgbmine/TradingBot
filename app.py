...
class FactorEngine:
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 2. Volatility Squeeze Confirmation
        ...
        # 28. Institutional Participation Proxy
    ...
    def composite_cis(self, factors: pd.DataFrame) -> pd.Series:
        ...
        score = (score / tot * 100 + 50).clip(0, 100) # normalized to 0-100 scale
...
class BacktestEngine:
    ...
    def run(self, ticker: str):
        try:
            ...
            # Simple trade simulation based on the 35 factors
            ...
            return {"df": df, "cis": cis, "audit": self.debugger.audit(f, cis), "trades": len(trades_df), "wr": wr, "ret": ret}
...
# ============================================================
# APP SCREENS (Wyckoff, VP, VWAP, Composite - Stubbed for Brevity)
# ============================================================
def screen_wyckoff():
    st.markdown("""<div class="header-box wyckoff"><h2>⬛ WYCKOFF ACCUMULATION</h2></div>""",unsafe_allow_html=True)
    st.info("מודול זה פעיל בקוד המקורי, השארתי כאן פלייסבולדר כדי לא להעמיס את הזיכרון. המסך המרכזי שביקשת הוא ה-Backtest.")
...
# ============================================================
# 🚀 BACKTEST SCREEN (WITH THE 35 FACTORS EXPLANATION)
# ============================================================
def screen_backtest():
    st.markdown("""
        ...
        with st.spinner(f"מעבד 35 אינדיקטורים מוסדיים על {ticker}..."):
            ...
            if "error" in res:
                ...
                with pc1:
                    ...
                    if positives:
                        for p in positives[:4]: # Show top 4 positive
                ...
                with pc2:
                    ...
                    if negatives:
                        for n in sorted(negatives, key=lambda x: x['impact'])[:4]: # Show top 4 negative
